...
//                    return v2.replace(/\d{1,3}(?=(\d{3})+$)/g, '$&,') + (v3.slice(0, _this.settings.decimal + 1) || '.00');
                    return v2.replace(/\d{1,3}(?=(\d{3})+$)/g, '$&,') + formatDecimal(v3,_this.settings.decimal);
                });
            }
            return s.replace(/^\./, "0.");
             ...
             * 格式化小数点
             */
            function formatDecimal(v3,decimal){
            	if(decimal>0){
            		var currentValue = v3.indexOf(".")>-1?v3:".";
                	var resultValue = currentValue;
                	for(var j=currentValue.length-1;j<decimal;j++){
                		resultValue+="0";
                	}
                	return resultValue;
            	}else{
